jQuery(function ($) {
  ...
  function draw_autodiscover_logs () {
    ft_autodiscover_logs = FooTable.init('#autodiscover_log', {
      'columns': [
        {
          'name': 'time', 'formatter': function unix_time_format (tm) {
            var date = new Date(tm ? tm * 1000 : 0)
            return date.toLocaleString()
          }, 'title': lang.time, 'style': { 'width': '170px' }
        },
      ...
      'columns': [
        { 'name': 'indicator', 'title': ' ', 'style': { 'width': '50px' } },
        {
          'name': 'time', 'formatter': function unix_time_format (tm) {
            var date = new Date(tm ? tm * 1000 : 0)
            return date.toLocaleString()
          }, 'title': lang.last_applied, 'style': { 'width': '170px' }
        },
  ...
  function draw_rspamd_history () {
    ft_rspamd_history = FooTable.init('#rspamd_history', {
      'columns': [
        {
          'name': 'unix_time', 'formatter': function unix_time_format (tm) {
            var date = new Date(tm ? tm * 1000 : 0)
            return date.toLocaleString()
          }, 'title': lang.time, 'style': { 'width': '170px' }
        },
        ...
        { 'name': 'rcpt', 'title': 'To', 'breakpoints': 'xs sm md', 'style': { 'minWidth': 100 } },
        {
          ...
          'title': 'Subject',
          'breakpoints': 'all',
          'style': { 'word-break': 'break-all', 'minWidth': 150 }
        },
    ...
    if (ft = FooTable.get($('#' + log_table))) {
      ...
      $.get('/api/v1/get/logs/' + log_url + '/' + load_rows).then(function (data) {
        if (data.length === undefined) {
          mailcow_alert_box(lang.no_new_rows, 'info')
          return
        }
