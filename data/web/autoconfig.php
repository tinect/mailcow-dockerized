...
<clientConfig version="1.1">
    <emailProvider id="<?= $mailcow_hostname; ?>">
        ...
        <?php
        $records = dns_get_record('_pop3s._tcp.' . $domain,
            DNS_SRV); // check if POP3 is announced as "not provided" via SRV record
        ...
        <?php } ?>
        <?php
        $records = dns_get_record('_pop3._tcp.' . $domain,
            DNS_SRV); // check if POP3 is announced as "not provided" via SRV record
        if (count($records) == 0 || $records[0]['target'] != '') { ?>
            <incomingServer type="pop3">
                <hostname><?= $autodiscover_config['pop3']['server']; ?></hostname>
        ...
        <enable visiturl="https://<?= $mailcow_hostname; ?><?php if ($port != 443) {
            echo ':' . $port;
        } ?>/admin.php">
            <instruction>If you didn't change the password given to you by the administrator or if you didn't change it
                in a long time, please consider doing that now.
            </instruction>
            <instruction lang="de">Sollten Sie das Ihnen durch den Administrator vergebene Passwort noch nicht geändert
                haben, empfehlen wir dies nun zu tun. Auch ein altes Passwort sollte aus Sicherheitsgründen geändert
                werden.
            </instruction>
        </enable>
    ...
    <webMail>
        <loginPage url="https://<?= $mailcow_hostname; ?><?php if ($port != 443) {
            echo ':' . $port;
        } ?>/SOGo/"/>
    </webMail>
</clientConfig>
