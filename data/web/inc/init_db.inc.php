                ...
                if ($num_results != 0) {
                    ...
                    if ($num_results != 0) {
                        ...
                        while ($row = array_shift($tls_options_rows)) {
                            $tls_options[$row['username']] = array(
                                'tls_enforce_in' => $row['tls_enforce_in'],
                                'tls_enforce_out' => $row['tls_enforce_out']
                            );
                        }
                    }
                }
      ...
      VALUES ('admin', '{SSHA256}K8eVJ6YsZbQCfuJvSUbaQRLr0HPLz5rC9IAp0PAFl0tmNDBkMDc0NDAyOTAxN2Rk', 1, NOW(), NOW(), 1)");
        ...
        `attributes` = JSON_SET(`attributes`, '$.tls_enforce_out', :tls_enforce_out)
          WHERE `username` = :username");
            $stmt->execute(array(
                ':tls_enforce_in' => $tls_options['tls_enforce_in'],
                ':tls_enforce_out' => $tls_options['tls_enforce_out'],
                ':username' => $tls_user
            ));
        }
